...
struct OMXCodecObserver : public BnOMXObserver {
    ...
    void setCodec(const sp<CiOMXCodec> &target) {
        mTarget = target;
    }

    // from IOMXObserver
    virtual void onMessage(const omx_message &msg) {
        sp<CiOMXCodec> codec = mTarget.promote();
...
// static
uint32_t CiOMXCodec::getComponentQuirks(
...
// static
void CiOMXCodec::findMatchingCodecs(
        const char *mime,
...
// static
sp<MediaSource> CiOMXCodec::Create(
        const sp<IOMX> &omx,
    ...
    for (size_t i = 0; i < matchingCodecs.size(); ++i) {
        ...
        if (err == OK) {
            ...
            sp<CiOMXCodec> codec = new CiOMXCodec(
...
status_t CiOMXCodec::configureCodec(const sp<MetaData> &meta) {
...
void CiOMXCodec::setMinBufferSize(OMX_U32 portIndex, OMX_U32 size) {
...
status_t CiOMXCodec::setVideoPortFormatType(
        OMX_U32 portIndex,
...
status_t CiOMXCodec::findTargetColorFormat(
...
status_t CiOMXCodec::isColorFormatSupported(
...
void CiOMXCodec::setVideoInputFormat(
...
status_t CiOMXCodec::setupErrorCorrectionParameters() {
...
status_t CiOMXCodec::setupBitRate(int32_t bitRate) {
...
status_t CiOMXCodec::getVideoProfileLevel(
        const sp<MetaData>& meta,
...
status_t CiOMXCodec::setupH263EncoderParameters(const sp<MetaData>& meta) {
...
status_t CiOMXCodec::setupMPEG4EncoderParameters(const sp<MetaData>& meta) {
    int32_t iFramesInterval, frameRate, bitRate;
    bool success = meta->findInt32(kKeyBitRate, &bitRate);
    success = success && meta->findInt32(kKeyFrameRate, &frameRate);
    success = success && meta->findInt32(kKeyIFramesInterval, &iFramesInterval);
...
status_t CiOMXCodec::setupAVCEncoderParameters(const sp<MetaData>& meta) {
    int32_t iFramesInterval, frameRate, bitRate;
    bool success = meta->findInt32(kKeyBitRate, &bitRate);
    success = success && meta->findInt32(kKeyFrameRate, &frameRate);
    success = success && meta->findInt32(kKeyIFramesInterval, &iFramesInterval);
...
status_t CiOMXCodec::setVideoOutputFormat(
...
// static
void CiOMXCodec::setComponentRole(
...
// static
bool CiOMXCodec::isIntermediateState(State state) {
...
status_t CiOMXCodec::allocateBuffers() {
...
status_t CiOMXCodec::allocateBuffersOnPort(OMX_U32 portIndex) {
    ...
    mDealer[portIndex] = new MemoryDealer(totalSize, "CiOMXCodec");
...
status_t CiOMXCodec::applyRotation() {
...
status_t CiOMXCodec::allocateOutputBuffersFromNativeWindow() {
...
status_t CiOMXCodec::cancelBufferToNativeWindow(BufferInfo *info) {
...
CiOMXCodec::BufferInfo* CiOMXCodec::dequeueBufferFromNativeWindow() {
...
status_t CiOMXCodec::pushBlankBuffersToNativeWindow() {
...
int64_t CiOMXCodec::retrieveDecodingTimeUs(bool isCodecSpecific) {
    CHECK(mIsEncoder);
...
void CiOMXCodec::on_message(const omx_message &msg) {
    if (mState == ERROR) {
...
void CiOMXCodec::onEvent(OMX_EVENTTYPE event, OMX_U32 data1, OMX_U32 data2) {
    switch (event) {
...
void CiOMXCodec::onCmdComplete(OMX_COMMANDTYPE cmd, OMX_U32 data) {
    switch (cmd) {
...
void CiOMXCodec::onStateChange(OMX_STATETYPE newState) {
    ...
    switch (newState) {
        ...
        case OMX_StateExecuting:
        {
            ...
            // call to CiOMXCodec::read as mInitialBufferSubmit is true at
...
// static
size_t CiOMXCodec::countBuffersWeOwn(const Vector<BufferInfo> &buffers) {
    size_t n = 0;
    for (size_t i = 0; i < buffers.size(); ++i) {
        if (buffers[i].mStatus != OWNED_BY_COMPONENT) {
            ++n;
...
status_t CiOMXCodec::freeBuffersOnPort(
...
status_t CiOMXCodec::freeBuffer(OMX_U32 portIndex, size_t bufIndex) {
    Vector<BufferInfo> *buffers = &mPortBuffers[portIndex];
...
void CiOMXCodec::onPortSettingsChanged(OMX_U32 portIndex) {
...
bool CiOMXCodec::flushPortAsync(OMX_U32 portIndex) {
    CHECK(mState == EXECUTING || mState == RECONFIGURING
...
void CiOMXCodec::disablePortAsync(OMX_U32 portIndex) {
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
...
status_t CiOMXCodec::enablePortAsync(OMX_U32 portIndex) {
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
...
void CiOMXCodec::fillOutputBuffers() {
    CHECK_EQ((int)mState, (int)EXECUTING);
...
void CiOMXCodec::drainInputBuffers() {
    CHECK(mState == EXECUTING || mState == RECONFIGURING);
...
bool CiOMXCodec::drainAnyInputBuffer() {
    return drainInputBuffer((BufferInfo *)NULL);
}

CiOMXCodec::BufferInfo *CiOMXCodec::findInputBufferByDataPointer(void *ptr) {
...
CiOMXCodec::BufferInfo *CiOMXCodec::findEmptyInputBuffer() {
    Vector<BufferInfo> *infos = &mPortBuffers[kPortIndexInput];
    for (size_t i = 0; i < infos->size(); ++i) {
        BufferInfo *info = &infos->editItemAt(i);
...
bool CiOMXCodec::drainInputBuffer(BufferInfo *info) {
    if (info != NULL) {
        CHECK_EQ((int)info->mStatus, (int)OWNED_BY_US);
    }
...
void CiOMXCodec::fillOutputBuffer(BufferInfo *info) {
    CHECK_EQ((int)info->mStatus, (int)OWNED_BY_US);
...
bool CiOMXCodec::drainInputBuffer(IOMX::buffer_id buffer) {
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexInput];
    for (size_t i = 0; i < buffers->size(); ++i) {
...
void CiOMXCodec::fillOutputBuffer(IOMX::buffer_id buffer) {
    Vector<BufferInfo> *buffers = &mPortBuffers[kPortIndexOutput];
    for (size_t i = 0; i < buffers->size(); ++i) {
        if ((*buffers)[i].mBuffer == buffer) {
            fillOutputBuffer(&buffers->editItemAt(i));
...
void CiOMXCodec::setState(State newState) {
    mState = newState;
...
status_t CiOMXCodec::waitForBufferFilled_l() {
...
void CiOMXCodec::setRawAudioFormat(
...
void CiOMXCodec::setAMRFormat(bool isWAMR, int32_t bitRate) {
...
status_t CiOMXCodec::setAACFormat(int32_t numChannels, int32_t sampleRate, int32_t bitRate) {
    if (numChannels > 2)
...
void CiOMXCodec::setG711Format(int32_t numChannels) {
    CHECK(!mIsEncoder);
    setRawAudioFormat(kPortIndexInput, 8000, numChannels);
}

void CiOMXCodec::setImageOutputFormat(
...
void CiOMXCodec::setJPEGInputFormat(
...
void CiOMXCodec::addCodecSpecificData(const void *data, size_t size) {
...
void CiOMXCodec::clearCodecSpecificData() {
...
status_t CiOMXCodec::start(MetaData *meta) {
...
sp<MetaData> CiOMXCodec::getFormat() {
    Mutex::Autolock autoLock(mLock);

    return mOutputFormat;
}

status_t CiOMXCodec::read(
...
void CiOMXCodec::signalBufferReturned(MediaBuffer *buffer) {
    Mutex::Autolock autoLock(mLock);
...
void CiOMXCodec::dumpPortStatus(OMX_U32 portIndex) {
    OMX_PARAM_PORTDEFINITIONTYPE def;
    InitOMXParams(&def);
    def.nPortIndex = portIndex;
...
status_t CiOMXCodec::initNativeWindow() {
...
void CiOMXCodec::initNativeWindowCrop() {
    int32_t left, top, right, bottom;
...
void CiOMXCodec::initOutputFormat(const sp<MetaData> &inputFormat) {
...
status_t QueryCodecs(
    ...
    CiOMXCodec::findMatchingCodecs(mime,
            !queryDecoders /*createEncoder*/,
            NULL /*matchComponentName*/,
            hwCodecOnly ? CiOMXCodec::kHardwareCodecsOnly : 0 /*flags*/,
            &matchingCodecs);
    ...
    for (size_t c = 0; c < matchingCodecs.size(); c++) {
        ...
        CiOMXCodec::setComponentRole(omx, node, !queryDecoders, mime);
...
void CiOMXCodec::restorePatchedDataPointer(BufferInfo *info) {
